...
class ListDetailSpider(object):
    def __init__(self, config, proj_dir=None, logger=None, coll_name = 'news01'):
        ...
        self.down_retry = 3
        if not logger:
            configure_logging("WX_processive_new.log")
    ...
    def start_run(self, begin_key=None, start_date="", end_date="", max_page=10, range_idx=None, start_page=1, industry=None, q=None):
        ...
        list_mongo = []
        for each_mongo in self.mongo_col2.find():
            record_each_mongo = {}
            _id = each_mongo['_id']
            _id_four = str(_id)[-4:]
        ...
        if list_mongo:
            ...
            for num_first, name_c in enumerate(list_mongo):
                                            ...
                                            try:
                                                ...
                                                if tag_time:
                                                    ...
                                                    if flag_time == 1:
                                                        ...
                                                        pulish_time_list.append(publish_time)
                ...
                self.update_mongo_publish_date_sort_function(_id, self.mongo_col2, pulish_time_list)
...
def run(db, coll_name, last_run_time, logger, *args):
    SAVE_MONGO_CONFIG["db"] = db
    bp = ListDetailSpider(SAVE_MONGO_CONFIG, logger=logger, coll_name=coll_name)
